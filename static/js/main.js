...
const state = {
    selectedModel: 'Gemini 2.5 Flash',
    selectedProvider: 'google',
    selectedApiName: 'gemini-2.5-flash',
    currentMediaType: 'llm',
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    modelSelector.dataset.mediaType = state.currentMediaType;
    document.getElementById('selectedModelName').textContent = state.selectedModel;
    document.getElementById('selectedModelApiName').textContent = state.selectedApiName;
    ...
    // Initial message area setup requires event delegation for dynamically created elements
    document.getElementById('chatMessages').addEventListener('click', (event) => {
        const sampleQuestionEl = event.target.closest('.sample-question');
        if (sampleQuestionEl) {
            sendSampleQuestion(sampleQuestionEl.dataset.question);
    ...
    // Initial state setup
    handleInput(); // Set initial button state and textarea size
    startNewChat(); // Populate the welcome screen
