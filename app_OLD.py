...
def format_model_data(row: dict[str, int|str]) -> dict[str, int | str | bool | None]:
    ...
    is_premium = False
    premium_icon = None
    input_price: int = row['usd_per_million_input_tokens']
...
def main() -> None:
    ...
    if not usersDbExists and not dataDbExists and not modelsDbExists:
        ...
        succ: dict[str, str] = db_lib.create_databases()
        db_lib.database_check_success("create", "both databases", succ, logger)
        ...
        succ: dict[str, str] = db_lib.database_fix_missing(logger)
        db_lib.database_check_success("fix missing", "missing databases", succ, logger)
